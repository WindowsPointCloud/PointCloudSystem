...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None):    
    ...
    for cur_thresh in [0.5]: # cfg.MODEL.POST_PROCESSING.RECALL_THRESH_LIST:
    ...
    with open(cfg.DATA_CONFIG._BASE_CONFIG_, 'r') as file:
         data = yaml.safe_load(file)
    MAP_CLASS_TO_KITTI = data['MAP_CLASS_TO_KITTI']
    print()
    ...
    for class_group in classes:
        for group in class_group:
            ...
            if "AP_R40" in first_string and "0.50" in first_string and "0.75" not in first_string:
                ...
                value = cls = group[0].split()[0]
                key = get_key_by_value(MAP_CLASS_TO_KITTI, value)
                #print(key, value)
                print(group[0].replace(value, key))
                print(group[1])
                print()
